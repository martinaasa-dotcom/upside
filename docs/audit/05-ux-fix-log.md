...
| M1 | Icon-only touch targets are 28px (`size-7` / `icon-sm`), below the ~44pt HIG and 48dp Material minimums | Medium | **Deferred** | — | A design-system decision, not a mechanical fix: `icon-sm` is defined once in `src/components/ui/button.tsx` and bumping it moves every icon button in the app. The highest-traffic offenders were separately fixed on `main` in f8d5114, which applied the existing `.touch-target` utility to `MobileTopBar`'s buttons, the View menu, Add-holding, the sheet picker, `UpgradeNudge`, and the shared Dialog close — so the phone-facing surfaces already meet 44×44. What remains is the systemic variant default, which needs Martin's call on whether every small icon button in the app grows. **Partly closed since:** `.touch-target` was gated on `@media (max-width: 1023px)`, a viewport query, so it missed every touch device wide enough to look like a desktop — an iPad in landscape, a touchscreen laptop — which is exactly where a 28px control is hardest to hit. It is now `(max-width: 1023px), (pointer: coarse)` (`src/app/globals.css`), so the surfaces already carrying the utility meet 44×44 on any device you tap, and no desktop pixel moves. |
