...
|---|---|---|---|---|---|
| H1 | R2 disaster-recovery cold copies have no retention policy and are never purged for a deleted account | High | **Resolved in code; R2 itself is not provisioned** | `src/lib/dr/config.ts` default is **30 days** (was 90), pinned by `src/lib/dr/config.test.ts` so it can't drift from the published number. `docs/DISASTER_RECOVERY.md` documents the 45-day R2 lifecycle rule as a backstop and a restore obligation, ready the moment cold storage exists. | Decision taken: 30 days, cron prune as primary, bucket lifecycle rule at 45 days as a backstop, for whenever cold storage is turned on. 30 because these copies exist to rebuild after catastrophic loss, a mass delete, or ransomware — all noticed in days, not months; 90 read as an archive, raising the GDPR bar for no operational gain. **Correction (2026-08-19):** this row originally said "Privacy policy §7 states 30 in all three places," describing the cold-copy backup as if it existed. It does not — Martin confirmed no Cloudflare/R2 account has ever been created, and `readDrConfig()` treats the four `DR_S3_*` vars as fully optional, so the disaster-recovery cron has been running nightly with cold storage silently skipped (`cold.skipped`, logged as a warning, never surfaced as a failure) since it shipped. Publishing a specific retention number for a backup channel that was never provisioned was a real defect, not a documentation nit. §7 has been rewritten to describe only what runs today (nightly in-database snapshots) and to promise nothing about an off-site copy until one exists. The 30-day config and the 45-day lifecycle-rule instructions stay in the codebase, ready to go live the day R2 (or S3) is actually set up — Martin has decided not to do that for now, given the required card-on-file. |
