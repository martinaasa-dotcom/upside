...
|---|---|---|---|---|---|
| M1 | Privacy Policy's retention section didn't disclose the R2 cold-copy backup channel | Medium | **Resolved** (prior session; briefly reverted and restored same day) | `src/app/privacy/page.tsx` §7; report §Medium 1 | Fixed when the pass was first run — this was the gap Pass 9 handed over. Briefly pulled on 2026-08-19 after it came out that R2 had never actually been provisioned (see Pass 9 H1's history note); restored once Martin created the bucket, lifecycle rule and API credentials and set the four `DR_S3_*` vars in Vercel. Not independently re-verified that a cold copy has actually landed in the bucket yet — see the same caveat on Pass 9 H1. |
