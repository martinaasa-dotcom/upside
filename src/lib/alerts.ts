  ...
  for (const r of rows) {
    if (r.stockTarget != null && r.spot > 0 && r.spot >= r.stockTarget) {
      out.push({
        ...
        kind: "strike",
        title: `${cashtag(r.ticker)} through Stock Target`,
    ...
    if (r.nextStrike != null && r.spot > 0 && r.spot >= r.nextStrike * 0.98) {
      out.push({
        ...
        kind: "strike",
        title: `${cashtag(r.ticker)} near planned Next Strike`,
...
export function buildEarningsAlerts(
  ...
  return events
    ...
    .map((e) => ({
      ...
      title: `${cashtag(e.ticker)} earnings in ${e.days}d`,
...
export function buildDecisionAlerts(input: {
  ...
  if (
    ...
    out.push({
      ...
      kind: "info",
      title: `${cashtag(input.topTicker.ticker)} is ${share.toFixed(0)}% of equity`,
