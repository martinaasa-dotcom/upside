...
function RankCard({
  ...
  return (
    <button
      ...
      onClick={onOpen}
      className={cn(
        "group relative flex w-full items-center gap-4 overflow-hidden rounded-2xl border px-4 py-4 text-left transition",
    ...
    >
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <div className="flex flex-wrap items-baseline gap-2">
          <span className="text-xl font-semibold text-white">{ticker.ticker}</span>
          <span className="text-sm text-zinc-400">
            {currency(ticker.currentValue, 0)} total
          </span>
        </div>
        <PortfolioChips names={ticker.portfolios} />
        <div className="flex items-center gap-3 text-sm text-zinc-500">
          <span>{ticker.shares.toLocaleString("en-US")} sh</span>
          <Sparkline points={ticker.sparkline} width={80} height={22} />
        </div>
      </div>
      <div className="shrink-0 text-right">
        <div className={cn("text-2xl font-bold tabular-nums", tone(metric))}>
          {percent(metric)}
        </div>
    ...
    <button type="button" onClick={onOpen} className="group w-full text-left">
      <div className="mb-2 flex items-end justify-between gap-3">
        ...
        <div className="shrink-0 text-right">
          ...
          <p className={cn("text-sm tabular-nums", tone(sheet.roiPct))}>
            {percent(sheet.roiPct)} · {signedCurrency(sheet.roiDollar)}
          </p>
        </div>
      </div>
      <div className="h-4 overflow-hidden rounded-full bg-zinc-800/80">
        <div
          className={cn(
...
export function OverviewDashboard({ model, onOpenSheet }: Props) {
  ...
  useEffect(() => {
    ...
    void fetch(
      ...
      .then((r) => r.json())
      .then((data: MarketEvents) => {
        if (!cancelled) setEvents(data);
      })
      .catch(() => {
        if (!cancelled) setEvents({ earnings: [], catalysts: [] });
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="space-y-7">
      {/* Hero */}
      <section className="overview-fade relative overflow-hidden rounded-3xl border border-zinc-800/80 bg-zinc-950/60 p-6 sm:p-8">
        <div
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-lg text-base text-zinc-400 sm:text-lg">
              {totals.sheetCount} portfolios · {totals.uniqueTickers} tickers ·{" "}
              {totals.positionCount} positions · tap a name to jump in.
            </p>
          </div>
          <div
            className={cn(
              "overview-pulse rounded-2xl border px-5 py-4",
              dayUp
        ...
        <div className="relative mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            >
              ...
              <p
                className={cn(
                  "mt-1 text-2xl font-semibold tabular-nums text-white sm:text-3xl",
      ...
      <section className="overview-fade rounded-3xl border border-zinc-800/80 bg-zinc-950/40 p-6 sm:p-7">
        <div className="mb-5">
          <h3 className="text-2xl font-semibold text-white">Portfolios</h3>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <div className="overview-fade rounded-3xl border border-emerald-500/20 bg-gradient-to-b from-emerald-500/5 to-zinc-950/40 p-6">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-2xl font-semibold text-white">Winners</h3>
              <p className="text-base text-zinc-400">
                Best lifetime ROI · one row per ticker
              </p>
            </div>
            <TrendingUp className="ml-auto h-5 w-5 text-emerald-400" />
          </div>
          <div className="space-y-3">
            {winners.length === 0 ? (
              <p className="rounded-xl border border-dashed border-zinc-800 px-3 py-8 text-center text-base text-zinc-500">
        ...
        <div className="overview-fade rounded-3xl border border-rose-500/20 bg-gradient-to-b from-rose-500/5 to-zinc-950/40 p-6">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-2xl font-semibold text-white">Losers</h3>
              <p className="text-base text-zinc-400">
                Deepest drawdowns · one row per ticker
              </p>
            </div>
            <TrendingDown className="ml-auto h-5 w-5 text-rose-400" />
          </div>
          <div className="space-y-3">
            {losers.length === 0 ? (
              <p className="rounded-xl border border-dashed border-zinc-800 px-3 py-8 text-center text-base text-zinc-500">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <div className="overview-fade rounded-3xl border border-zinc-800/80 bg-zinc-950/40 p-6">
          ...
          <p className="mb-5 text-base text-zinc-400">
            Session gainers · deduped across portfolios
          </p>
          <div className="space-y-3">
            {todayWinners.length === 0 ? (
              <p className="text-base text-zinc-500">Waiting on live quotes…</p>
            ) : (
        ...
        <div className="overview-fade rounded-3xl border border-zinc-800/80 bg-zinc-950/40 p-6">
          ...
          <p className="mb-5 text-base text-zinc-400">
            Session laggards · deduped across portfolios
          </p>
          <div className="space-y-3">
      ...
      <section className="overview-fade rounded-3xl border border-zinc-800/80 bg-zinc-950/40 p-6 sm:p-7">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-base text-zinc-400">
              Combined market value across all portfolios
            </p>
          </div>
        </div>
        <div className="space-y-2.5">
          {topHoldings.map((t, i) => (
            <button
              ...
              className="flex w-full flex-wrap items-center gap-3 rounded-2xl border border-zinc-800/80 bg-zinc-900/30 px-4 py-3.5 text-left transition hover:border-zinc-600 hover:bg-zinc-900/60 sm:flex-nowrap"
            >
              <span className="w-8 text-base tabular-nums text-zinc-500">
                {i + 1}
              </span>
              <div className="min-w-0 flex-1 space-y-1.5">
                <div className="flex flex-wrap items-baseline gap-2">
              ...
              <div className="ml-auto text-right">
                ...
                <p className={cn("text-sm tabular-nums", tone(t.roiPct))}>
        ...
        <div className="overview-fade rounded-3xl border border-zinc-800/80 bg-zinc-950/40 p-6">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base text-zinc-400">
                Earnings dates + theme watchpoints
              </p>
            </div>
          </div>
          <div className="space-y-2.5">
            {!events ? (
              <p className="text-base text-zinc-500">Loading catalysts…</p>
            ) : events.catalysts.length === 0 ? (
              <p className="text-base text-zinc-500">
                No catalysts lined up for owned names.
              </p>
            ) : (
              events.catalysts.slice(0, 12).map((c, i) => (
                <div
                  key={`${c.ticker}-${c.label}-${i}`}
                  className="flex items-start justify-between gap-3 rounded-2xl border border-zinc-800/80 bg-zinc-900/30 px-4 py-3"
                >
                  <div>
                    <p className="text-lg font-semibold text-white">
                      {c.ticker}
                      <span className="ml-2 text-sm font-normal text-zinc-400">
                        {c.kind === "earnings" ? "Earnings" : "Theme"}
                      </span>
                    </p>
                    <p className="mt-0.5 text-base text-zinc-300">{c.label}</p>
                  </div>
                  <div className="shrink-0 text-right text-sm text-zinc-400">
                    {c.date ? (
                      <>
                        <p className="tabular-nums text-zinc-200">{c.date}</p>
                        <p>
                          {c.days === 0
                            ? "Today"
                            : c.days === 1
        ...
        <div className="overview-fade rounded-3xl border border-zinc-800/80 bg-zinc-950/40 p-6">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base text-zinc-400">
                Next report dates for owned tickers
              </p>
            </div>
          </div>
          <div className="space-y-2.5">
            {!events ? (
              <p className="text-base text-zinc-500">Loading earnings…</p>
            ) : events.earnings.length === 0 ? (
              <p className="text-base text-zinc-500">
                No earnings dates found in the next 90 days.
              </p>
            ) : (
              events.earnings.map((e) => {
                ...
                return (
                  <div
                    key={e.ticker}
                    className="rounded-2xl border border-zinc-800/80 bg-zinc-900/30 px-4 py-3"
                  >
                    <div className="flex items-baseline justify-between gap-3">
                      <p className="text-xl font-semibold text-white">
                        {e.ticker}
                      </p>
                      <p className="text-base tabular-nums text-zinc-200">
                        {e.date}
      ...
      <section className="overview-fade rounded-3xl border border-amber-500/20 bg-gradient-to-br from-amber-500/10 via-zinc-950/40 to-zinc-950/40 p-6 sm:p-7">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-2xl font-semibold text-white">Fun facts</h3>
            <p className="text-base text-zinc-400">
              Auto-roasted from your live books
            </p>
          </div>
        </div>
        <ul className="space-y-3">
          {funFacts.length === 0 ? (
            <li className="text-base text-zinc-500">
              Load some quotes and the facts will show up.
            </li>
          ) : (
            funFacts.map((fact) => (
              <li
                key={fact}
                className="rounded-2xl border border-zinc-800/70 bg-zinc-950/50 px-4 py-3.5 text-base leading-relaxed text-zinc-200 sm:text-lg"
              >
                {fact}
              </li>
            ))
