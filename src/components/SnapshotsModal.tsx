...
export function SnapshotsModal({ open, onClose, onRestored }: Props) {
  ...
  async function restore(id: string, label: string) {
    if (
      !window.confirm(
        `Restore “${label}”? This replaces the entire live book. A safety snapshot is taken first.`
      )
    ) {
      return;
    }
        ...
        <div className="flex items-start justify-between gap-3 border-b border-zinc-800 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-500">
              Nightly backups + pre-delete safety copies. Restore needs your
              owner PIN.
            </p>
          </div>
          <button
                  ...
                  {snapshots.map((s) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() => void restore(s.id, s.label || s.kind)}
                        className="shrink-0 rounded-md bg-brand/15 px-2.5 py-1.5 text-xs font-semibold text-brand-bright hover:bg-brand/25 disabled:opacity-50"
                      >
                        {busyId === s.id ? "…" : "Restore"}
                      </button>
                    </li>
